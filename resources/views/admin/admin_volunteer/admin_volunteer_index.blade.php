...
<div id="dataTable_wrapper" class="dataTables_wrapper dt-bootstrap4">
  ...
  <!-- ค้นหา -->
    ...
    <form method="GET" action="{{ url('/admin_volunteer') }}" accept-charset="UTF-8" class="form-inline my-2 my-lg-0 float-right" role="search">
        ...
        <table class="table dataTable">
		  ...
		  @foreach($users as $item)
        	<tbody>
              ...
              <td> {{ $item->email  }}</td>
              <td> {{ $item->role  }}</td>
              <td><a href="{{ url('/admin_volunteer/' . $item->id) }}" title="View Detail"><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i>ประวัติส่วนตัว</button></a>
